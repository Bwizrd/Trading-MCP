...
async def list_backtest_json_files() -> list[TextContent]:
    ...
    try:
        # Search for JSON files in optimization_results directory
        json_pattern = "optimization_results/backtest_*.json"
...
async def create_chart_from_backtest_json(json_filename: str) -> list[TextContent]:
    ...
    Args:
        json_filename: Name or path of the JSON file. Can be:
                      ...
                      - Relative path: "optimization_results/backtest_EURUSD_20251101_133004.json"  
    ...
    async with get_initialized_components() as (connector, registry, engine, chart_eng):
        try:
            ...
            if os.path.isabs(json_filename):
                ...
                json_filepath = os.path.join("optimization_results", json_filename)
            
            if not os.path.exists(json_filepath):
                # Try to provide helpful suggestions
                import glob
                available_files = glob.glob("optimization_results/backtest_*.json")
                if available_files:
